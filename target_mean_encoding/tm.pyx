...
cpdef target_mean_v3(data, y_name, x_name):
    ...
    cdef np.ndarray[double] result = np.asfortranarray(np.zeros(data.shape[0]), dtype=np.float64)
...
cdef void target_mean_v4_impl(double[:] result, int[:] y, double[:] x,  const long nrow):
    # use unordered_map
